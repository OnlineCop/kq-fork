...
static void bufferize_ex(const int which_map)
{
    ...
    if (which_map == 1)
    {
        size_t map_size = gmap1.xsize * gmap1.ysize;
        free(map1);
        map1 = (unsigned short *) calloc(map_size, ssize);
        free(b_map1);
        b_map1 = (unsigned short *) calloc(map_size, ssize);
        free(f_map1);
        f_map1 = (unsigned short *) calloc(map_size, ssize);
        free(o_map1);
        o_map1 = (unsigned char *) calloc(map_size, csize);
        free(s_map1);
        s_map1 = (unsigned char *) calloc(map_size, csize);
        free(z_map1);
        z_map1 = (unsigned char *) calloc(map_size, csize);
    }
    else if (which_map == 2)
    {
        size_t map_size = gmap2.xsize * gmap2.ysize;
        free(map2);
        map2 = (unsigned short *) calloc(map_size, ssize);
        free(b_map2);
        b_map2 = (unsigned short *) calloc(map_size, ssize);
        free(f_map2);
        f_map2 = (unsigned short *) calloc(map_size, ssize);
        free(o_map2);
        o_map2 = (unsigned char *) calloc(map_size, csize);
        free(s_map2);
        s_map2 = (unsigned char *) calloc(map_size, csize);
        free(z_map2);
        z_map2 = (unsigned char *) calloc(map_size, csize);
    }
}                               /* bufferize_ex() */
...
{
    int i, j;
    int _chrx, _x, _y, _tilex, _tiley, _eid, _active, _facing, _moving, _movcnt, _framectr, _movemode, _obsmode, _delay, _delayctr, _speed, _scount, _cmd, _sidx, _extra, _chasing, _cmdnum, _atype, _snapback, _facehero, _transl, _script;
    ...
    for (i = 0; i < 50; ++i)
    {
        ...
        if ((_chrx) || (_x) || (_y) || (_tilex) || (_tiley) || (_eid) || (_active) || (_facing) || (_moving) || (_movcnt) || (_framectr) || (_movemode) || (_obsmode) || (_delay) || (_delayctr) || (_speed) || (_scount) || (_cmd) || (_sidx) || (_extra) || (_chasing) || (_cmdnum) || (_atype) || (_snapback) || (_facehero) || (_transl) || (_script))
        {
        ...
        if (_tilex)
        {
            fprintf(stdout, "  tilex:   \t%d\t\t%d\n", gent1[i].tilex, gent2[i].tilex);
        }
        if (_tiley)
        {
            fprintf(stdout, "  tiley:   \t%d\t\t%d\n", gent1[i].tiley, gent2[i].tiley);
        }
        if (_eid)
        {
            fprintf(stdout, "  eid:     \t%d\t\t%d\n", gent1[i].eid, gent2[i].eid);
        }
        if (_active)
        {
            fprintf(stdout, "  active:  \t%d\t\t%d\n", gent1[i].active, gent2[i].active);
        }
        if (_facing)
        {
            fprintf(stdout, "  facing:  \t%d\t\t%d\n", gent1[i].facing, gent2[i].facing);
        }
        if (_moving)
        {
            fprintf(stdout, "  moving:  \t%d\t\t%d\n", gent1[i].moving, gent2[i].moving);
        }
        if (_movcnt)
        {
            fprintf(stdout, "  movcnt:  \t%d\t\t%d\n", gent1[i].movcnt, gent2[i].movcnt);
        }
        if (_framectr)
        {
            fprintf(stdout, "  framectr:\t%d\t\t%d\n", gent1[i].framectr, gent2[i].framectr);
        }
        if (_movemode)
        {
            fprintf(stdout, "  movemode:\t%d\t\t%d\n", gent1[i].movemode, gent2[i].movemode);
        }
        if (_obsmode)
        {
            fprintf(stdout, "  obsmode: \t%d\t\t%d\n", gent1[i].obsmode, gent2[i].obsmode);
        }
        if (_delay)
        {
            fprintf(stdout, "  delay:   \t%d\t\t%d\n", gent1[i].delay, gent2[i].delay);
        }
        if (_delayctr)
        {
            fprintf(stdout, "  delayctr:\t%d\t\t%d\n", gent1[i].delayctr, gent2[i].delayctr);
        }
        if (_speed)
        {
            fprintf(stdout, "  speed:   \t%d\t\t%d\n", gent1[i].speed, gent2[i].speed);
        }
        if (_scount)
        {
            fprintf(stdout, "  scount:  \t%d\t\t%d\n", gent1[i].scount, gent2[i].scount);
        }
        if (_cmd)
        {
            fprintf(stdout, "  cmd:     \t%d\t\t%d\n", gent1[i].cmd, gent2[i].cmd);
        }
        if (_sidx)
        {
            fprintf(stdout, "  sidx:    \t%d\t\t%d\n", gent1[i].sidx, gent2[i].sidx);
        }
        if (_extra)
        {
            fprintf(stdout, "  extra:   \t%d\t\t%d\n", gent1[i].extra, gent2[i].extra);
        }
        if (_chasing)
        {
            fprintf(stdout, "  chasing: \t%d\t\t%d\n", gent1[i].chasing, gent2[i].chasing);
        }
        if (_cmdnum)
        {
            fprintf(stdout, "  cmdnum:  \t%d\t\t%d\n", gent1[i].cmdnum, gent2[i].cmdnum);
        }
        if (_atype)
        {
            fprintf(stdout, "  atype:   \t%d\t\t%d\n", gent1[i].atype, gent2[i].atype);
        }
        if (_snapback)
        {
            fprintf(stdout, "  snapback:\t%d\t\t%d\n", gent1[i].snapback, gent2[i].snapback);
        }
        if (_facehero)
        {
            fprintf(stdout, "  facehero:\t%d\t\t%d\n", gent1[i].facehero, gent2[i].facehero);
        }
        if (_transl)
        {
            fprintf(stdout, "  transl:  \t%d\t\t%d\n", gent1[i].transl, gent2[i].transl);
        }
        if (_script)
        {
            fprintf(stdout, "  scripts: \t%s\t\t%s\n", gent1[i].script, gent2[i].script);
        }
    }
}                               /* check_entities() */
    ...
    if (_map && _map < 120)
    {
        ...
        for (j = 0; j < gmap1.ysize; j++)
        {
            for (k = 0; k < gmap1.xsize; k++)
            {
                ...
                if ((unsigned short) map1[i] != (unsigned short) map2[i])
                {
                    sprintf(strbuf, "[(%d, %d) %d/%d]", k, j, (unsigned short) map1[i], (unsigned short) map2[i]);
                    if (strlen(strbuf) < 16)
                    {
                        fprintf(stdout, "%s\t\t", strbuf);
                    }
    ...
    if (_bmap && _bmap < 120)
    {
        ...
        for (j = 0; j < gmap1.ysize; j++)
        {
            for (k = 0; k < gmap1.xsize; k++)
            {
                ...
                if ((unsigned short) b_map1[i] != (unsigned short) b_map2[i])
                {
                    sprintf(strbuf, "[(%d, %d) %d/%d]", k, j, (unsigned short) b_map1[i], (unsigned short) b_map2[i]);
                    if (strlen(strbuf) < 16)
                    {
                        fprintf(stdout, "%s\t\t", strbuf);
                    }
    ...
    if (_fmap && _fmap < 120)
    {
        ...
        for (j = 0; j < gmap1.ysize; j++)
        {
            for (k = 0; k < gmap1.xsize; k++)
            {
                ...
                if ((unsigned short) f_map1[i] != (unsigned short) f_map2[i])
                {
                    sprintf(strbuf, "[(%d, %d) %d/%d]", k, j, (unsigned short) f_map1[i], (unsigned short) f_map2[i]);
                    if (strlen(strbuf) < 16)
                    {
                        fprintf(stdout, "%s\t\t", strbuf);
                    }
    ...
    if (_omap && _omap < 120)
    {
        ...
        for (j = 0; j < gmap1.ysize; j++)
        {
            for (k = 0; k < gmap1.xsize; k++)
            {
                ...
                if ((unsigned char) o_map1[i] != (unsigned char) o_map2[i])
                {
                    sprintf(strbuf, "[(%d, %d) %d/%d]", k, j, (unsigned char) o_map1[i], (unsigned char) o_map2[i]);
                    if (strlen(strbuf) < 16)
                    {
                        fprintf(stdout, "%s\t\t", strbuf);
                    }
    ...
    if (_smap && _smap < 120)
    {
        ...
        for (j = 0; j < gmap1.ysize; j++)
        {
            for (k = 0; k < gmap1.xsize; k++)
            {
                ...
                if ((unsigned char) s_map1[i] != (unsigned char) s_map2[i])
                {
                    sprintf(strbuf, "[(%d, %d) %d/%d]", k, j, (unsigned char) s_map1[i], (unsigned char) s_map2[i]);
                    if (strlen(strbuf) < 16)
                    {
                        fprintf(stdout, "%s\t\t", strbuf);
                    }
    ...
    if (_zmap && _zmap < 120)
    {
        ...
        for (j = 0; j < gmap1.ysize; j++)
        {
            for (k = 0; k < gmap1.xsize; k++)
            {
                ...
                if ((unsigned char) z_map1[i] != (unsigned char) z_map2[i])
                {
                    sprintf(strbuf, "[(%d, %d) %d/%d]", k, j, (unsigned char) z_map1[i], (unsigned char) z_map2[i]);
                    if (strlen(strbuf) < 16)
                    {
                        fprintf(stdout, "%s\t\t", strbuf);
                    }
...
void check_map(void)
{
    ...
    if ((_map_no) || (_zero_zone) || (_map_mode) || (_can_save) || (_tileset) || (_use_sstone) || (_can_warp) || (_extra_byte) || (_xsize) || (_ysize) || (_pmult) || (_pdiv) || (_stx) || (_sty) || (_warpx) || (_warpy) || (_revision) || (_extra_sdword2) || (_song_file) || (_map_desc) || (_num_markers) || (_num_bound_boxes))
    {
    ...
    if (_map_no)
    {
        fprintf(stdout, "  map_no:     \t%d\t\t%d\n", gmap1.map_no, gmap2.map_no);
    }
    if (_zero_zone)
    {
        fprintf(stdout, "  zero_zone:  \t%d\t\t%d\n", gmap1.zero_zone, gmap2.zero_zone);
    }
    if (_map_mode)
    {
        fprintf(stdout, "  map_mode:   \t%d\t\t%d\n", gmap1.map_mode, gmap2.map_mode);
    }
    if (_can_save)
    {
        fprintf(stdout, "  can_save:   \t%d\t\t%d\n", gmap1.can_save, gmap2.can_save);
    }
    if (_tileset)
    {
        fprintf(stdout, "  tileset:    \t%d\t\t%d\n", gmap1.tileset, gmap2.tileset);
    }
    if (_use_sstone)
    {
        fprintf(stdout, "  use_sstone: \t%d\t\t%d\n", gmap1.use_sstone, gmap2.use_sstone);
    }
    if (_can_warp)
    {
        fprintf(stdout, "  can_warp:   \t%d\t\t%d\n", gmap1.can_warp, gmap2.can_warp);
    }
    if (_extra_byte)
    {
        fprintf(stdout, "  extra_byte: \t%d\t\t%d\n", gmap1.extra_byte, gmap2.extra_byte);
    }
    if (_xsize)
    {
    ...
    if (_extra_sdword2)
    {
        fprintf(stdout, "  extra_sdword2:%d\t\t%d\n", gmap1.extra_sdword2, gmap2.extra_sdword2);
    }
    if (_song_file)
    {
    ...
    while (marker_num < (_num_markers1 > _num_markers2 ? _num_markers1 : _num_markers2))
    {
        if (marker_num < gmap1.markers.size && marker_num < gmap2.markers.size)
        {
            ...
            if ((_m1[marker_num].x != _m2[marker_num].x) || (_m1[marker_num].y != _m2[marker_num].y) || (strcmp(_m1[marker_num].name, _m2[marker_num].name) != 0))
            {
    ...
    if (gmap1.revision >= 2 && gmap2.revision >= 2)
    {
        ...
        {
            if (bound_box_num < gmap1.bounds.size && bound_box_num < gmap2.bounds.size)
            {
...
void usage(const char *argv)
{
    ...
    fprintf(stdout, "  Output will be any differences found between the two maps.\n");
}                               /* usage() */
