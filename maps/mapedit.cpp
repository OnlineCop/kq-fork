...
s_anim tanim[NUM_TILESETS][MAX_ANIM] =
{
    ...
    /* newtown.pcx */
    {{158, 159, 50}, {160, 163, 25}, {176, 179, 25}, {257, 258, 50}, {262, 263, 25}},
    /* castle.pcx */
...
void clear_layer(void)
{
    ...
    while (!done)
    {
        ...
        if (strlen(strbuf) > 0)
        {
            ...
            {
                sprintf(strbuf, "Do you want to clear Layer %d? (y/n)", selected_layer);
                cmessage(strbuf);
                if (yninput())
                {
                    done = 1;
...
void draw_map(s_show &showing)
{
    ...
    for (dy = 0; dy < maxy; dy++)
    {
        for (dx = 0; dx < maxx; dx++)
        {
            ...
            {
                rectfill(double_buffer, dx * TILE_W, dy * TILE_H, (dx + 1) * TILE_W - 1, (dy + 1) * TILE_H - 1, 0);
            }
            ...
            {
                draw_trans_sprite(double_buffer, shadow[sh_map[w]], dx * TILE_W, dy * TILE_H);
            }

            /* Draw the Obstacles */
            assert(o_map[w] <= MAX_OBSTACLES && "o_map[w] >= MAX_OBSTACLES\n");
            if ((showing.obstacles) && (o_map[w] > 0) && (o_map[w] <= MAX_OBSTACLES))
            {
            ...
            if ((showing.zones) && (z_map[w] > 0))
            {
                if (z_map[w] < 10)
                {
                    ...
                    textprintf_ex(double_buffer, font, dx * TILE_W + 4, dy * TILE_H + 4, makecol(255, 255, 255), 0, "%d", z_map[w]);
                }
                else if (z_map[w] < 100)
                {
                    /* The zone's number is double-digit, center only vert */
                    textprintf_ex(double_buffer, font, dx * TILE_W, dy * TILE_H + 4, makecol(255, 255, 255), 0, "%d", z_map[w]);
                }
                else
                {
                     ...
                     * of the square
                     */
                    textprintf_ex(double_buffer, font, dx * TILE_W + 4, dy * TILE_H, makecol(255, 255, 255), 0, "%d", (int)(z_map[w] / 100));
                    textprintf_ex(double_buffer, font, dx * TILE_W, dy * TILE_H + TILE_H / 2, makecol(255, 255, 255), 0, "%02d", (int)(z_map[w] % 100));
                }
            }                      // if (showing.zones)
            ...
            if (highlight)
            {
                if ((draw_mode == MAP_SHADOWS && sh_map[w] == curshadow && curshadow > 0) || (draw_mode == MAP_OBSTACLES && o_map[w] == curobs && curobs > 0) || (draw_mode == MAP_ZONES && z_map[w] == curzone && curzone > 0))
                {
    ...
    if (showing.entities == 1)
    {
        for (i = 0; i < number_of_ents; i++)
        {
            ...
            if ((gent[i].tilex >= window_x) && (gent[i].tilex < window_x + htiles) && (gent[i].tiley >= window_y) && (gent[i].tiley < window_y + vtiles))
            {
                /* Draw either a normal sprite or a translucent one */
                if (gent[i].transl == 0)
                {
                    draw_sprite(double_buffer, eframes[gent[i].chrx][gent[i].facing * ENT_FRAMES_PER_DIR], (gent[i].tilex - window_x) * TILE_W, (gent[i].tiley - window_y) * TILE_H);
                }
                else
                {
                    draw_trans_sprite(double_buffer, eframes[gent[i].chrx][gent[i].facing * ENT_FRAMES_PER_DIR], (gent[i].tilex - window_x) * TILE_W, (gent[i].tiley - window_y) * TILE_H);
                }                   // if..else ()
    ...
    if (draw_mode == BLOCK_COPY && copyx1 != -1 && copyy1 != -1)
    {
        ...
        if (copying == 0)
        {
            ...
            rect(double_buffer, x1 - 1, y1 - 1, x2 + 1, y2 + 1, 25);
        }
        else
        {
            /* Only the initial tile is selected */
            rect(double_buffer, x1 - 1, y1 - 1, x1 + TILE_W, y1 + TILE_H, 25);
        }
    }
    ...
    if (showing.boundaries)
    {
        ...
        if (active_bound)
        {
            ...
            rect(double_buffer, x1, y1, x2, y2, 41);
        }
    }
}                               /* draw_map() */
...
void draw_menubars(s_show &showing)
{
    ...
    char dt[][12] =
    {
        "Layer1", "Layer2", "Layer3", "View L1+2", "View L1+3", "View L2+3", "View L1+2+3", "Shadows", "Zones", "Obstacles", "Entities", "Block Copy", "Block Paste", "Preview", "Markers", "Bounds",
    };
    ...
    print_sfont(column[2], row[5], gmap.use_sstone == 0 ? "SunStone: NO" : "SunStone: YES", double_buffer);
    ...
    else if (draw_mode == MAP_OBSTACLES)
    {
        if (curobs > 0)
        {
            sprintf(strbuf, "Obstacle #%d (found: %d)", curobs, count_current_obstacles());
        }
        else
        {
        ...
        xp = mouse_tile_x + window_x;
        yp = mouse_tile_y + window_y;
        if ((xp - window_x) < htiles && (yp - window_y) < vtiles)
        {
            p = o_map[(yp * gmap.xsize) + xp];
    ...
    else if (draw_mode == MAP_SHADOWS)
    {
        if (curshadow > 0)
        {
            sprintf(strbuf, "Shadow #%d (found: %d)", curshadow, count_current_shadows());
        }
        else
        {
    ...
    else if (draw_mode == MAP_ZONES)
    {
        if (curzone > 0)
        {
            sprintf(strbuf, "Zone #%d (found: %d)", curzone, count_current_zones());
        }
        else
        {
    ...
    {
        sprintf(strbuf, "Bounding Area #%d: (%d, %d) (%d, %d), tile=%d", curbound_box, gmap.bounds.array[curbound_box].left, gmap.bounds.array[curbound_box].top, gmap.bounds.array[curbound_box].right, gmap.bounds.array[curbound_box].bottom, gmap.bounds.array[curbound_box].btile);
        print_sfont(column[4], row[0], strbuf, double_buffer);
        xp = mouse_tile_x + window_x;
        yp = mouse_tile_y + window_y;
        for (i = 0; i < gmap.bounds.size; i++)
    ...
    {
        sprintf(strbuf, "Copy From:: (%d, %d)", (copyx1 > -1 ? copyx1 : 0), (copyy1 > -1 ? copyy1 : 0));
        print_sfont(column[4], row[0], strbuf, double_buffer);

        sprintf(strbuf, "Copy To: (%d, %d)", (copyx2 > -1 ? copyx2 : 0), (copyy2 > -1 ? copyy2 : 0));
        print_sfont(column[4], row[1], strbuf, double_buffer);
    }
    ...
    if (curtile >= icon_set * ICONSET_SIZE && curtile < (icon_set + 2) * ICONSET_SIZE)
    {
    ...
    if (draw_the_rect)
    {
        rect(double_buffer, (xp + htiles) * TILE_W + TILE_W / 2 + 1, yp * TILE_H + TILE_H / 2 - 1, (xp + htiles + 1) * TILE_W + TILE_W / 2 + 1, (yp + 1) * TILE_H + TILE_H / 2 - 1, 255);
    }
    ...
    if (draw_mode == MAP_ENTITIES)
    {
        ...
        stretch_blit(eframes[current_ent][0], double_buffer, 0, 0, TILE_W, TILE_H, (htiles + 1) * TILE_W + TILE_W / 2, 250, TILE_W * 2, TILE_H * 2);
    }
    else if (draw_mode == MAP_OBSTACLES)
    {
        ...
        if (curobs > 0)
        {
            stretch_blit(mesh1[curobs - 1], double_buffer, 0, 0, TILE_W, TILE_H, (htiles + 1) * TILE_W + TILE_W / 2, 250, TILE_W * 2, TILE_H * 2);
        }
    }
    else if (draw_mode == MAP_SHADOWS)
    {
        sprintf(strbuf, "Shadow");
        stretch_blit(shadow[curshadow], double_buffer, 0, 0, TILE_W, TILE_H, (htiles + 1) * TILE_W + TILE_W / 2, 250, TILE_W * 2, TILE_H * 2);
    }
    else if (draw_mode == MAP_MARKERS)
    {
        sprintf(strbuf, "Markers");
        stretch_blit(marker_image, double_buffer, 0, 0, TILE_W, TILE_H, (htiles + 1) * TILE_W + TILE_W / 2, 250, TILE_W * 2, TILE_H * 2);
    ...
    if ((mouse_tile_x < htiles) && (mouse_tile_y < vtiles))
    {
    ...
    if (draw_mode == MAP_ENTITIES)
    {
        ...
        blit(eframes[current_ent][0], double_buffer, 0, 0, (htiles + 4) * TILE_W, (SH - 32), TILE_W, TILE_H);
    ...
    if (mouse_tile_x < htiles && mouse_tile_y < vtiles)
    {
        rect(double_buffer, x * TILE_W, y * TILE_H, (x + 1) * TILE_W - 1, (y + 1) * TILE_H - 1, 255);
    }
    ...
    if (draw_mode == MAP_OBSTACLES || draw_mode == MAP_SHADOWS || draw_mode == MAP_ZONES || draw_mode == MAP_MARKERS || draw_mode == MAP_BOUNDS)
    {
...
int find_cursor(int direction)
{
    ...
    for (row = 0; row < gmap.ysize; row++)
    {
        for (col = 0; col < gmap.xsize; col++)
        {
            ...
            if (the_attrib == 0 || search_map[row * gmap.xsize + col] != the_attrib)
            {
                continue;
            }
...
void get_tile(void)
{
    ...
    switch (draw_mode)
    {
        ...
        case MAP_ENTITIES:
            ...
            {
                if ((gent[tile].tilex == xx) && (gent[tile].tiley == yy))
                {
        ...
        case MAP_BOUNDS:
            ...
            found_box = is_contained_bound(gmap.bounds.array, gmap.bounds.size, xx, yy, xx, yy);
            if (found_box != NULL)
            {
...
void make_rect(BITMAP *where, const int rect_height, const int rect_width)
{
    ...
    rectfill(where, 0, 0, ((rect_width + 1) * 6) + 4, ((rect_height + 1) * 6) + 4, 0);
    rect(where, 2, 2, ((rect_width + 1) * 6) + 2, ((rect_height + 1) * 6) + 2, 255);
}                               /* make_rect() */
...
void preview_map(void)
{
    ...
    switch (gmap.map_mode)
    {
        ...
        default:
            ...
            textprintf_centre_ex(double_buffer, font, double_buffer->w / 2, double_buffer->h / 2, makecol(255, 255, 255), 0, "Mode %d preview not supported.", gmap.map_mode);
            break;
    }
}                               /* preview_map() */
...
int process_keyboard(s_show &showing, const int k)
{
    ...
    switch (k)
    {
        ...
        case (KEY_G):
             ...
             */
            if ((draw_mode >= MAP_LAYER1 && draw_mode <= MAP_LAYER3) || (draw_mode >= MAP_ENTITIES && draw_mode <= MAP_ZONES) || (draw_mode == MAP_MARKERS || draw_mode == MAP_BOUNDS))
            {
...
void process_menu_bottom(const int cx, const int cy)
{
    ...
    {
        rectfill(double_buffer, column[0] + 6 * 6, row[2], column[2] - 1, row[3] - 1, 0);
        print_sfont(column[0] + 6 * 5, row[2], ">", double_buffer);
        hline(double_buffer, column[0] + 6 * 6, row[3] - 1, column[0] + 6 * 24 - 1, 255);
        blit2screen();
    ...
    {
        rectfill(double_buffer, column[0] + 6 * 7, row[4], column[1] - 1, row[5] - 1, 0);
        print_sfont(column[0] + 6 * 6, row[4], ">", double_buffer);
        hline(double_buffer, column[0] + 6 * 7, row[5] - 1, column[0] + 6 * 10 - 1, 255);
        blit2screen();
    ...
    {
        rectfill(double_buffer, column[1] + 6 * 7, row[5], column[2] - 1, row[6] - 1, 0);
        print_sfont(column[1] + 6 * 6, row[5], ">", double_buffer);
        hline(double_buffer, column[1] + 6 * 7, row[6] - 1, column[1] + 6 * 11 - 1, 255);
        blit2screen();
    ...
    {
        rectfill(double_buffer, column[1] + 6 * 7, row[6], column[2] - 1, row[7] - 1, 0);
        print_sfont(column[1] + 6 * 6, row[6], ">", double_buffer);
        hline(double_buffer, column[1] + 6 * 7, row[7] - 1, column[1] + 6 * 11 - 1, 255);
        blit2screen();
    ...
    {
        rectfill(double_buffer, column[2] + 6 * 9, row[1], column[3] - 1, row[2] - 1, 0);
        print_sfont(column[2] + 6 * 8, row[1], ">", double_buffer);
        hline(double_buffer, column[2] + 6 * 9, row[2] - 1, column[2] + 6 * 13 - 1, 255);
        blit2screen();
    ...
    {
        rectfill(double_buffer, column[2] + 6 * 9, row[2], column[3] - 1, row[3] - 1, 0);
        print_sfont(column[2] + 6 * 8, row[2], ">", double_buffer);
        hline(double_buffer, column[2] + 6 * 9, row[3] - 1, column[2] + 6 * 13 - 1, 255);
        blit2screen();
    ...
    {
        rectfill(double_buffer, column[3] + 6 * 6, row[1], column[4] - 1, row[2] - 1, 0);
        print_sfont(column[3] + 6 * 5, row[1], ">", double_buffer);
        hline(double_buffer, column[3] + 6 * 6, row[2] - 1, column[3] + 6 * 9 - 1, 255);
        blit2screen();
    ...
    {
        rectfill(double_buffer, column[3] + 6 * 5, row[2], column[4] - 1, row[3] - 1, 0);
        print_sfont(column[3] + 6 * 4, row[2], ">", double_buffer);
        hline(double_buffer, column[3] + 6 * 5, row[3] - 1, column[3] + 6 * 8 - 1, 255);
        blit2screen();
...
void process_menu_right(const int current_mouse_x, const int current_mouse_y)
{
    ...
    if (cx > (htiles + 1) * TILE_W + 6 && cx <= (htiles + 3) * TILE_W + 9 && cy > 248 && cy < 284)
    {
        if ((draw_mode >= MAP_ENTITIES && draw_mode <= MAP_ZONES) || draw_mode == MAP_MARKERS || draw_mode == MAP_BOUNDS)
        {
            find_cursor(0);
        }
        else
    ...
    if ((cx >= (htiles + 1) * TILE_W && cx < (htiles + 4) * TILE_W) && (cy >= 316 && cy < 364))
    {
        switch (draw_mode)
        {
            case MAP_BOUNDS:
            ...
            case MAP_ZONES:
                 ...
                 */
                find_cursor((((cy - 316) / TILE_H) * 3 + (cx - ((htiles + 1) * TILE_W)) / TILE_W) - 4);
                break;
        }
    }
        ...
        if (dmode == 1)
        {
            switch (draw_mode)
            {
                ...
                case (MAP_ENTITIES):
                    ...
                    place_entity((mouse_x / TILE_W) + window_x, (mouse_y / TILE_H) + window_y);
                    break;
...
void read_controls(s_show &showing)
{
    ...
    if (mouse_b & 1)
    {
        ...
        {
            /* Draw to the map */
            if ((draw_mode >= MAP_LAYER1 && draw_mode <= MAP_ZONES) || (draw_mode == MAP_MARKERS) || (draw_mode == MAP_BOUNDS))
            {
                dmode = 1;
            }
...
void resize_map(const int selection)
{
    ...
    if (done > 0)
    {
        ...
        sprintf(strbuf, "%d marker%s will be discarded! Continue? (y/n)", done, done == 1 ? "" : "s");
        cmessage(strbuf);
...
// *INDENT-ON*
    ...
    rectfill(double_buffer, i - 5, j - 5, i + (strlen(*help_keys) * FW) + 4, j + (NUMBER_OF_ITEMS * (FH + 1)) + 4, 0);
    rect(double_buffer, i - 3, j - 3, i + (strlen(*help_keys) * FW) + 2, j + (NUMBER_OF_ITEMS * (FH + 1)) + 2, 255);
    ...
    vline(double_buffer, i + (strlen(*help_keys) * FW / 2) - 1, j + ((FH + 1) * 3) - 1, j + (NUMBER_OF_ITEMS - 2) * (FH + 1) - 2, 255);
    blit2screen();
    yninput();
...
void update_tileset(void)
{
    ...
    for (row = 0; row < max_sets; row++)
    {
        ...
        {
            blit(pcx_buffer, icons[row * ICONSET_SIZE + col], col * TILE_W, row * TILE_H, 0, 0, TILE_W, TILE_H);
        }
    }
    icon_set = 0;
