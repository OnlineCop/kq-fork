...
static void draw_equipmenu(int c, int sel)
{
	...
	Draw.print_font(double_buffer, 268 - (strlen(party[l].name) * 4) + xofs, 76 + yofs,
	...
	for (k = 0; k < NUM_EQUIPMENT; k++)
	{
		...
		Draw.print_font(double_buffer, 92 + xofs, k * 8 + 36 + yofs, items[j].name,
			FNORMAL);
	}
}
...
static void draw_equippable(uint32_t c, uint32_t slot, uint32_t pptr)
{
	...
	for (k = 0; k < sm; k++)
	{
		...
		Draw.print_font(double_buffer, 36 + xofs, k * 8 + 100 + yofs, items[j].name,
			FNORMAL);
		if (z > 1)
		{
			sprintf(strbuf, "^%d", z);
			Draw.print_font(double_buffer, 164 + xofs, k * 8 + 100 + yofs, strbuf,
				FNORMAL);
		}
	}
	if (pptr > 0)
...
static void draw_equippreview(int ch, int ptr, int pp)
{
	...
	for (z = 0; z < 13; z++)
	{
		...
		c2 = tstats[z];
		sprintf(strbuf, "%d", c1);
		Draw.print_font(double_buffer, 252 - (strlen(strbuf) * 8) + xofs,
		...
		if (ptr >= 0)
		{
			sprintf(strbuf, "%d", c2);
			if (c1 < c2)
				Draw.print_font(double_buffer, 300 - (strlen(strbuf) * 8) + xofs,
	...
	if (ptr >= 0)
	{
		...
		if (c1 < c2)
			Draw.print_font(double_buffer, 212 + xofs, 220 + yofs, _("Resist up"),
				FNORMAL);
		if (c1 > c2)
			Draw.print_font(double_buffer, 204 + xofs, 220 + yofs, _("Resist down"),
				FNORMAL);
	}
}
