...
static int KQ_move_entity(lua_State *L)
{
    ...
    find_path(entity_id,
        g_ent[entity_id].tilex,
        g_ent[entity_id].tiley,
        target_x,
        target_y,
        buffer,
        sizeof(buffer)
...
static int KQ_set_btile(lua_State *L)
{
    ...
    else
    {
         ...
         *    set_btile(x, y, value)
         */
        set_btile((int) lua_tonumber(L, 1),
            (int) lua_tonumber(L, 2),
            (int) lua_tonumber(L, 3)
        );
    }
...
static int KQ_set_ftile(lua_State *L)
{
    ...
    else
    {
         ...
         *    set_ftile(x, y, value)
         */
        set_ftile((int) lua_tonumber(L, 1),
            (int) lua_tonumber(L, 2),
            (int) lua_tonumber(L, 3)
        );
    }
...
static int KQ_set_mtile(lua_State *L)
{
    ...
    else
    {
         ...
         *    set_mtile(x, y, value)
         */
        set_mtile((int) lua_tonumber(L, 1),
            (int) lua_tonumber(L, 2),
            (int) lua_tonumber(L, 3)
        );
    }
...
static int KQ_set_shadow(lua_State *L)
{
    ...
    else
    {
         ...
         */
        set_shadow((int) lua_tonumber(L, 1),
            (int) lua_tonumber(L, 2),
            (int) lua_tonumber(L, 3)
        );
    }
...
static int KQ_make_sprite(lua_State*L) {
  ...
  if (nel >= 1) {
    ...
    BITMAP* bm = get_cached_image(filename);
    if (nel == 1) {
      // Push x, y, w, h
      lua_pushnumber(L, 0);
      lua_pushnumber(L, 0);
      lua_pushnumber(L, bm ? bm->w : 0);
      lua_pushnumber(L, bm ? bm->h : 0);
    } else if (nel == 5) {
...
static int KQ_drawsprite(lua_State* L) {
  if (lua_istable(L, 1)) {
    ...
    lua_pushliteral(L, "bitmap");
    lua_rawget(L, 1);
    BITMAP* bm = reinterpret_cast<BITMAP*>(lua_touserdata(L, -1));
    lua_pop(L, 1);
