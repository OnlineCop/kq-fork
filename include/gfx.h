...
public:
  ...
  Raster(Raster &&);
  ~Raster();
  void blitTo(Raster *target, int16_t src_x, int16_t src_y, uint16_t src_w,
              uint16_t src_h, int16_t dest_x, int16_t dest_y, uint16_t dest_w,
              uint16_t dest_h, bool masked);
  void blitTo(Raster *target, int16_t src_x, int16_t src_y, uint16_t dest_x,
              uint16_t dest_y, uint16_t src_w, uint16_t src_h, bool masked);
  void blitTo(Raster *target, int16_t src_x, int16_t src_y, int16_t dest_x,
              int16_t dest_y, uint16_t src_w, uint16_t src_h);
  ...
  uint8_t &ptr(int16_t x, int16_t y) { return data[x + y * stride]; }
...
inline void blit(Raster *src, Raster *dest, int sx, int sy, int dx, int dy,
                 int w, int h) {
  src->blitTo(dest, sx, sy, dx, dy, w, h, false);
}
inline void masked_blit(Raster *src, Raster *dest, int sx, int sy, int dx,
                        int dy, int w, int h) {
  src->blitTo(dest, sx, sy, dx, dy, w, h, true);
}
inline void stretch_blit(Raster *src, Raster *dest, int sx, int sy, int sw,
...
inline void vline(Raster *dest, uint16_t x, uint16_t y0, uint16_t y1,
                  uint8_t color) {
  dest->vline(x, y0, y1, color);
}
inline void hline(Raster *dest, uint16_t x0, uint16_t y, uint16_t x1,
                  uint8_t color) {
  dest->hline(x0, x1, y, color);
}
inline void clear_to_color(Raster *r, int c) { r->fill(c); }
inline void clear_bitmap(Raster *r) { r->fill(0); }
inline void putpixel(Raster *r, int x, int y, int c) { r->setpixel(x, y, c); }
