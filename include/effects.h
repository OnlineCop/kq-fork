...
class KEffects
{
  public:
     ...
     *
     * \param   target_fighter_index Target index within fighter[] array.
     * \param   multiple_target Multiple target flag.
     ...
     *
     * Draw the casting sprite.  Effect is x and y centered.
     *
     * One suggestion I received was to have the casting sprite stay on screen until the actual spell effect is done.
     *
     * I may yet implement this.
     *
     ...
     *
     * Calling the function requires supplying exact coordinates, so there is no need to worry about centering here...
     * the orient var (from the effect structure) is used to determine whether to draw under or over the fighters.
     *
     ...
     * \param   target_fighter_index Target index in fighter[] array.
     * \param   fighter_index Character indiex in fighter[] array doing the attacking.
     * \param   multiple_target 1 to target multiple, 0 to target single.
     */
    void fight_animation(size_t target_fighter_index, size_t fighter_index, int multiple_target);

    /*! \brief Fighter status.
     *
     * Just make sure the fighter in question is dead or not.
     * Sometimes, we still want to return true if s/he is dead.
     *
